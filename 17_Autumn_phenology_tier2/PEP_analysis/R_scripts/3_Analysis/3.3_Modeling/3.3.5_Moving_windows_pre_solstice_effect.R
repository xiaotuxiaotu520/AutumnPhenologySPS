...
for(k in 1:length(PEPdataList)){
  ...
  for (i in 1:length(variables)){
    ...
    variable.names=c(paste0(variables[i],".LO.SOm30"), paste0(variables[i],".LO.SO"),
                     paste0(variables[i],".LO.SOp30"), paste0(variables[i],".LO.SOp60"))
    ...
    for (Year in YearList[[k]]){
      ...
      presolstice.df = PEP.df.sub %>%
        ...
        mutate(period_end = ifelse(period == paste0(variables[i],".LO.SOm30"), 172-30, 
                                   ifelse(period == paste0(variables[i],".LO.SO"), 172, 
                                          ifelse(period == paste0(variables[i],".LO.SOp30"), 172+30, 172+60))))
