...
#############################################################################################################
# Figure 3 and Supplementary Figs. 5-7 and 10  ##############################################################
#############################################################################################################
...
for (variable.name in variable){
  ...
  #Plot
  plotG = ggplot(data=Predictions.df2, aes(y=value, x=MATanomaly, color=name, linetype=name)) +
    geom_hline(yintercept=0)+
  ...
  SolsticeMM.df.sub = SeasonalMM.df  %>% 
    filter(variable == variable.name &
             species == 'Aall' &
             grepl("Solstice.tempCon.scaled",equation) )
  ...
  plotD = plotD + annotation_custom(ggplotGrob(PhotoSolstice), xmin = 0.5, xmax = 4.5, 
  ...
  plotE = plotE + annotation_custom(ggplotGrob(PhotoSeasonal), xmin = 0.5, xmax = 5.5, 
                         ymin = 0.01, ymax = yRange[2])
  ...
  # Plot
  plotH = ggplot(data = FullMM.df.sub, aes(x = term, y = estimate, fill=term)) + 
    geom_bar(stat = "identity")+
    geom_errorbar(aes(ymin = estimate - 2*std.error, ymax = estimate + 2*std.error), width=.2,
                  position=position_dodge(.9)) +
    geom_hline(yintercept=0)+
  ...
  plotF.short = ggplot(MWreversal.df.sub, aes(x = year, y = date0, group=species, color=species)) + 
    ...
    coord_cartesian(xlim=c(1981.5,2000.08), ylim=c(as.Date('2000-06-05'),as.Date('2000-07-20')))+
  ...
  plotC = 
    ...
    coord_cartesian(xlim=c(1967.3,1994.7),ylim=c(-0.18,0.18))+
    xlab("") + ylab('')+
    scale_x_continuous(breaks = seq(1966,1996,by=5),
                       labels = c('','','','','','',''))+
    plotTheme1+
  ...
  #plot
  plotC.short = ggplot(MW.df.sub, aes(x = year, y = estimate, ymin = estimate-2*std.error, ymax = estimate+2*std.error,
                          group=term, color=term)) + 
    geom_hline(yintercept=0)+
    geom_ribbon(fill = "darkgrey", color=NA, alpha = 0.6) + 
    geom_line(size = 1) +
  ...
  #plot 
  plotI = ggplot(MW.df.full, aes(x = year, y = estimate, ymin = estimate-2*std.error, ymax = estimate+2*std.error,
                                group=term, color=term)) + 
    geom_hline(yintercept=0)+
    geom_ribbon(fill = "darkgrey", color=NA, alpha = 0.6) + 
    geom_line(size = 1) +
  ...
  #plot 
  plotI.species = ggplot(MW.df.full, aes(x = year, y = estimate, ymin = estimate-2*std.error, ymax = estimate+2*std.error,
                                 group=term, color=term)) + 
    geom_hline(yintercept=0)+
    geom_ribbon(fill = "darkgrey", color=NA, alpha = 0.6) + 
    geom_line(size = 1) +
  ...
  #plot 
  plotI.short = ggplot(MW.df.full, aes(x = year, y = estimate, ymin = estimate-2*std.error, ymax = estimate+2*std.error,
                                 group=term, color=term)) + 
    geom_hline(yintercept=0)+
    geom_ribbon(fill = "darkgrey", color=NA, alpha = 0.6) + 
    geom_line(size = 1) +
...
GHI"
  ...
  #Merge plots
  Fig1_Plot = plotA + plotB + plotC + plotD + plotE + plotF + plotG + plotH + plotI +
    plot_layout(design = layout) + plot_annotation(tag_levels = 'a')&
    theme(plot.tag = element_text(face = 'bold'))
  
  #save plots as .pdf
  ggsave(Fig1_Plot, file=paste('Fig1.',variable.name, ".pdf", sep=''), path=output_path,
         width=11, height=11)
  
  
  ##########################################
  # Species-specific moving window (Fig. Sx)
  ##########################################
  ...
  #Merge plots
  Supp_Plot =  plotC.species + plotI.species +
    plot_layout(design = layout) + plot_annotation(tag_levels = 'a')&
    theme(plot.tag = element_text(face = 'bold'))
  ...
  #Merge plots
  Supp_Plot2 =  plotC.short + plotF.short + plotI.short +
    plot_layout(design = layout) + plot_annotation(tag_levels = 'a')&
    theme(plot.tag = element_text(face = 'bold'))
  
  #save plots as .pdf
  ggsave(Supp_Plot2, file=paste('MW.short.',variable.name, ".pdf", sep=''), path=output_path,
         width=5.5, height=9)
...
#  [1] pracma_2.3.3      broom_0.7.8       patchwork_1.1.1   wesanderson_0.3.6 data.table_1.14.0 forcats_0.5.1    
#[7] stringr_1.4.0     dplyr_1.0.7       purrr_0.3.4       readr_1.4.0       tidyr_1.1.3       tibble_3.1.2     
#[13] ggplot2_3.3.4     tidyverse_1.3.1  

#loaded via a namespace (and not attached):
#  [1] Rcpp_1.0.6        lubridate_1.7.10  lattice_0.20-44   assertthat_0.2.1  digest_0.6.27     utf8_1.2.1       
#[7] R6_2.5.0          cellranger_1.1.0  plyr_1.8.6        backports_1.2.1   reprex_2.0.0      evaluate_0.14    
#[13] httr_1.4.2        pillar_1.6.1      rlang_0.4.11      readxl_1.3.1      rstudioapi_0.13   Matrix_1.3-3     
#[19] rmarkdown_2.9     labeling_0.4.2    splines_4.1.0     munsell_0.5.0     compiler_4.1.0    modelr_0.1.8     
#[25] xfun_0.24         pkgconfig_2.0.3   mgcv_1.8-35       htmltools_0.5.1.1 tidyselect_1.1.1  fansi_0.5.0      
#[31] crayon_1.4.1      dbplyr_2.1.1      withr_2.4.2       grid_4.1.0        nlme_3.1-152      jsonlite_1.7.2   
#[37] gtable_0.3.0      lifecycle_1.0.0   DBI_1.1.1         magrittr_2.0.1    scales_1.1.1      cli_2.5.0        
#[43] stringi_1.6.2     farver_2.1.0      fs_1.5.0          xml2_1.3.2        ellipsis_0.3.2    generics_0.1.0   
#[49] vctrs_0.3.8       tools_4.1.0       glue_1.4.2        hms_1.1.0         yaml_2.2.1        colorspace_2.0-1 
#[55] rvest_1.0.0       knitr_1.33        haven_2.4.1  
