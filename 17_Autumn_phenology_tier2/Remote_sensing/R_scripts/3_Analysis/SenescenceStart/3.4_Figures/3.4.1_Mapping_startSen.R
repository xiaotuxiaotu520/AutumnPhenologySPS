...
#############################################################################################################
# Figure 2, Extended Data Figs. 1 and 3, Supplementary Figs. S1 and S3 ######################################
#############################################################################################################
...
PhenoMixed.df <- Pheno.df %>%
  ...
  filter(SWrad.LO.SO < quantile(.$SWrad.LO.SO, 0.99),
         ...
         GPPstart.LO.SO < quantile(.$GPPstart.LO.SO, 0.99),
         GPPstart.LO.SO > quantile(.$GPPstart.LO.SO, 0.01),
         Azani.LO.SO < quantile(.$Azani.LO.SO, 0.99),
         Azani.LO.SO > quantile(.$Azani.LO.SO, 0.01),
...
for(variable.name in variables) {
  ...
  DensityPlot = ggplot(Analysis.df.sub, aes(x=Pre, y=Post) )  +
    ...
    coord_cartesian(ylim = c(-.9, .9),xlim = c(-.9, .9))+
  ...
  HistoPlot = ggplot(Analysis.df.sub2, aes(x=estimate, fill=term)) +
    ...
    coord_cartesian(xlim = c(-.9, .9), ylim = c(11, 270))+
    plotTheme1 +
  ...
  plotFullLC = ggplot(data = Analysis.df.sub, aes(x = term, y = mean, fill=term)) + 
    ...
    geom_hline(yintercept=0)+
    xlab("") + ylab("Standardized effect") +
    coord_cartesian(ylim = yRange2) +
    scale_fill_manual(values = c('#F21A00','grey60','grey35','black','#3B9AB2'))+
    scale_x_discrete(labels = c('Out-Sol','Prcp pre','Prcp post','CO2','Sol-Off'))+
    plotTheme1 +
